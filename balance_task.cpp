// Ziyuweng Apr. 30 2017 a simple program to lift up a robot leg
/*============================================================================
==============================================================================
...
init_balance_task(void)
{
  ...
  if (firsttime){
    ...
    delta_x = my_vector(1, N_CART);//velocity in Cartisian space
    delta_th = my_vector(1, N_DOFS);//velocity in joint space
...
run_balance_task(void)
{
  ...
  case ASSIGN_COG_TARGET:
    ...
    cog_target.x[_X_] =0.06;
    cog_target.x[_Y_] = 0.015;
    cog_target.x[_Z_] = 0.12;
  ...
  case MOVE_TO_COG_TARGET: // this is for inverse kinematics control
   ...
   // inverseDynamicsFloat(delta_t, stat, TRUE, joint_des_state, NULL, NULL, fc);
    //This is usually not work,so I shall use the following inverse kinematic method
    //first calculate the kinematics of cog and than calculate the joint space velocity delta_th
    ...
    for (i=1; i<=N_DOFS; ++i) {
      ...
      joint_des_state[i].th = joint_des_state[i].th+delta_t*delta_th[i];
